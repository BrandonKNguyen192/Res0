...
export async function seed(orgId) {
  ...
  // Roles mirror the Auth0 role claims (and the demo personas) exactly.
  await createMember(orgId, { name: 'Brandon Nguyen', email: 'brandon@sohohospitality.co', role: 'owner' });
  await createMember(orgId, { name: 'Maria Santos', email: 'maria@sohohospitality.co', role: 'general_manager', venueId: above.id });
  await createMember(orgId, { name: 'Priya Anand', email: 'priya@sohohospitality.co', role: 'beverage_director' });
  await createMember(orgId, { name: 'David Chen', email: 'david@sohohospitality.co', role: 'server', venueId: above.id });
}
